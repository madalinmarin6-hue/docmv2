...
const t = {
  EN: {
    ...
    businessTitle: "Business Collaboration",
    businessDesc: "We are open to collaborating with companies and teams that use document processing tools on a daily basis. If your business handles frequent conversions, edits, or document management, we can create a special plan tailored to your needs — with unlimited usage, priority support, and custom integrations.",
    businessCta: "Contact us at support@docm.app to discuss a custom business plan.",
    cats: [
  ...
  RO: {
    ...
    businessTitle: "Colaborare cu Firme",
    businessDesc: "Suntem deschisi sa colaboram cu companii si echipe care folosesc instrumente de procesare a documentelor zilnic. Daca afacerea ta implica conversii frecvente, editari sau management de documente, putem crea un plan special adaptat nevoilor tale — cu utilizare nelimitata, suport prioritar si integrari personalizate.",
    businessCta: "Contacteaza-ne la support@docm.app pentru a discuta un plan de business personalizat.",
    cats: [
  ...
  return (
    <div className={`min-h-screen ${cm ? "bg-white text-black" : "bg-gradient-to-br from-[#0b1333] via-[#070b22] to-[#1a0b2e] text-white"}`}>
      ...
      <div className="pt-28 pb-20 px-6 max-w-6xl mx-auto">
        ...
        <div className={`p-8 rounded-2xl mb-20 ${cm ? "bg-indigo-50 border border-indigo-200" : "bg-indigo-500/5 border border-indigo-400/20"}`}>
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className={`text-xl font-bold mb-2 ${cm ? "text-indigo-800" : "text-indigo-300"}`}>{c.businessTitle}</h3>
              <p className={`text-sm leading-relaxed mb-3 ${cm ? "text-indigo-700" : "text-indigo-200/70"}`}>{c.businessDesc}</p>
              <p className={`text-sm font-semibold ${cm ? "text-indigo-600" : "text-indigo-400"}`}>{c.businessCta}</p>
            </div>
          </div>
        </div>
