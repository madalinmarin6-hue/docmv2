...
export default function PngViewerPage() {
  usePing()
  ...
  return (
    <ToolLayout title="PNG Viewer" subtitle="View PNG images with transparency support">
      {!imageUrl ? (
        <FileUploader
          accept=".png"
          label="Open a PNG image"
          sublabel="Supports .png files with transparency"
          onFile={loadFile}
          cloudFilterTypes={["png"]}
        />
      ) : (
        <div className={`flex flex-col h-[88vh] rounded-xl overflow-hidden border ${dm ? "border-[#404040]" : "border-gray-300"}`}>
          ...
          <div className={`flex items-center px-3 py-1 ${mbg} border-t text-[10px] ${mc}`}>
            ...
            <div className="flex items-center gap-1">
              ...
              <input type="range" min={10} max={400} value={zoom} onChange={e => setZoom(Number(e.target.value))} className="w-24 accent-cyan-500" />
