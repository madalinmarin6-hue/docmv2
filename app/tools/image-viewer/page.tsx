...
export default function ImageViewerPage() {
  ...
  const loadFile = useCallback(async (file: File) => {
    ...
    setImageUrl(url)
    // Auto-enable checker bg for PNG files (transparency)
    if (file.name.toLowerCase().endsWith(".png")) setCheckerBg(true)
    const img = new Image()
  ...
  const reset = () => {
    ...
    setImageUrl(""); setFileName(""); setFileSize(0); setDimensions({ w: 0, h: 0 }); fileRef.current = null; setCheckerBg(false)
  }

  return (
    <ToolLayout title="Image Viewer" subtitle="View JPG, PNG, WebP, BMP, SVG and other images">
      {!imageUrl ? (
        <FileUploader
          accept=".jpg,.jpeg,.png,.webp,.bmp,.svg,.gif,.tiff,.ico"
          label="Open an image"
          sublabel="Supports JPG, PNG, WebP, BMP, SVG, GIF, TIFF, ICO"
          onFile={loadFile}
          cloudFilterTypes={["jpg", "jpeg", "png", "webp", "bmp", "svg", "gif", "tiff", "ico"]}
        />
      ) : (
        <div className={`flex flex-col h-[88vh] rounded-xl overflow-hidden border ${dm ? "border-[#404040]" : "border-gray-300"}`}>
          <div className={`flex items-center gap-1 px-3 py-1.5 ${tbg} border-b`}>
            ...
            <button onClick={() => setCheckerBg(!checkerBg)} className={`${bb} ${checkerBg ? "text-cyan-400" : ""}`} title="Toggle transparency grid">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" /></svg>
            </button>
          ...
          <div className={`flex-1 overflow-auto ${dm ? "bg-[#1e1e1e]" : "bg-[#e8eaed]"} flex items-center justify-center p-4`}
            style={checkerBg ? { backgroundImage: `linear-gradient(45deg, ${dm ? "#2a2a2a" : "#d4d4d4"} 25%, transparent 25%), linear-gradient(-45deg, ${dm ? "#2a2a2a" : "#d4d4d4"} 25%, transparent 25%), linear-gradient(45deg, transparent 75%, ${dm ? "#2a2a2a" : "#d4d4d4"} 75%), linear-gradient(-45deg, transparent 75%, ${dm ? "#2a2a2a" : "#d4d4d4"} 75%)`, backgroundSize: "20px 20px", backgroundPosition: "0 0, 0 10px, 10px -10px, -10px 0px" } : {}}>
